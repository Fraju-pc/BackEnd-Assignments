...
//		return response;
	...
	public PetParkData savePetPark(Long contributorId, PetParkData petParkData) {
		...
		for(Amenity amenity : amenities) {
			amenity.getPetPark().add(petPark);
	...
	private PetPark findPetParkById(Long petParkId) {
		...
		return petParkDao.findById(petParkId).orElseThrow(() -> new NoSuchElementException("Pet poark with Id= " + petParkId + " does not exist."));
	}
	...
	public PetParkData retrievePetParkById(Long contributorId, Long parkId) {
		...
		if(petPark.getContributor().getContributorId() != contributorId) {
			throw new IllegalStateException("Pet park with Id= " + parkId + " is not owned by contributor Id= " + contributorId);
